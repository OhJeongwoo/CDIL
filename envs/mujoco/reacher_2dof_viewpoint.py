...
class Reacher2DOFViewpointCornerEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        self.steps = 0
        self.i_episode = 0
        scale = np.sqrt(2)
        ...
        self.offset = np.pi * 30 / 180.0
        utils.EzPickle.__init__(self)
        mujoco_env.MujocoEnv.__init__(self, 'reacher_2dof.xml', 2)
        self.viewer = None
...
#       
    ...
    def set_state_from_obs(self, obs):
        ...
        theta = np.arctan2(sin_theta, cos_theta) - self.offset
...
class Reacher2DOFViewpointWallEnv(mujoco_env.MujocoEnv, utils.EzPickle):
    def __init__(self):
        self.steps = 0
        self.i_episode = 0
        scale = np.sqrt(2)
